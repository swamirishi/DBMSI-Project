...
public abstract class BTreeFileI<I extends ID, T extends Tuple,K> extends IndexFile<I,K>
  ...
  public abstract BTIndexPageSupplier<I,T> getBTIndexPageSupplier();
	public abstract BTLeafPageSupplier<I,T> getBTLeafPageSupplier();
  ...
  private BTreeHeaderPageI<I,T> headerPage;
   ...
   *          of this btree file.
   */
  public BTreeHeaderPageI<I,T> getHeaderPage() {
    return headerPage;
  }
   ...
   */
  public BTreeFileI(String filename)
    throws GetFileEntryException,
	   PinPageException,
	   ConstructPageException
    {
      ...
      headerPage= getBTreeHeaderPageSupplier().getBTreeHeaderPage(headerPageId);
  ...
  public BTreeFileI(String filename, int keytype,
	...
	{
	  headerPage= getBTreeHeaderPageSupplier().getBTreeHeaderPage();
  ...
  private void  _destroyFile(PageId pageno) 
	   ...
	   FreePageException
    {
      ...
      BTSortedPageI<I,T> sortedPage;
      Page page=pinPage(pageno) ;
      sortedPage= getBTSortedPageSupplier().getBTSortedPage(page, headerPage.get_keyType());
      
      if (sortedPage.getType() == NodeType.INDEX) {
	BTIndexPageI<I,T> indexPage= getBTIndexPageSupplier().getBTIndexPage(page, headerPage.get_keyType());
  ...
  private void  updateHeader(PageId newRoot)
	     ...
	     UnpinPageException
    {
      ...
      BTreeHeaderPageI<I,T> header;
      PageId old_data;
      
      
      header= getBTreeHeaderPageSupplier().getBTreeHeaderPage( pinPage(headerPageId));
   ...
   */    
  public void insert(KeyClass key, I rid)
    ...
    {
      IKeyDataEntry<I>  newRootEntry;
      
      if (BTI.getKeyLength(key) > headerPage.get_maxKeySize())
	...
	throw new KeyNotMatchException(null,"");
      ...
      if ( trace != null )
	{
	  trace.writeBytes( "INSERT " + rid.getPageNo() + " "
			    + rid.getSlotNo() + " " + key + lineSep);
	...
	BTLeafPageI<I,T> newRootPage;
	I dummyrid;
	
	newRootPage=getBTLeafPageSupplier().getBTLeafPage(headerPage.get_keyType());
      ...
      if (newRootEntry != null)
	{
	  ...
	  newRootPage = getBTIndexPageSupplier().getBTIndexPage(headerPage.get_keyType());
	  newRootPageId=newRootPage.getCurPage();
  ...
  private IKeyDataEntry<I>  _insert(KeyClass key, I rid,
	    ...
	    
    {
      ...
      BTSortedPageI<I,T> currentPage;
      Page page;
      IKeyDataEntry<I> upEntry;
      
      
      page=pinPage(currentPageId);
      currentPage= getBTSortedPageSupplier().getBTSortedPage(page, headerPage.get_keyType());
	...
	BTIndexPageI<I,T>  currentIndexPage=getBTIndexPageSupplier().getBTIndexPage(page,
	...
	currentIndexPage= getBTIndexPageSupplier().getBTIndexPage(pinPage(currentPageId),
					   headerPage.get_keyType() );
	...
	if (currentIndexPage.available_space() >=
	    BTI.getKeyDataLength( upEntry.key, NodeType.INDEX))
	  {
	...
	BTIndexPageI<I,T> newIndexPage;
	PageId       newIndexPageId;
	
	// we have to allocate a new INDEX page and
	// to redistribute the index entries
	newIndexPage= getBTIndexPageSupplier().getBTIndexPage(headerPage.get_keyType());
	...
	I firstRid= getIDSupplier().getID();
	IKeyDataEntry<I> undoEntry=null;
	...
	if ( currentIndexPage.available_space() < 
	  ...
	  currentIndexPage.deleteSortedRecord 
	    (getIDSupplier().getID(currentIndexPage.getCurPage(),
	...
	if (BTI.keyCompare( upEntry.key, tmpEntry.key) >=0 )
	  {
	  ...
	  tmpEntry =
	    BTI.getEntryFromBytes(currentIndexPage.getpage(),
				 currentIndexPage.getSlotOffset(i),
				 currentIndexPage.getSlotLength(i),
				 headerPage.get_keyType(),NodeType.INDEX,getIDSupplier(),getLeafDataSupplier(),getKeyDataEntrySupplier());

	  newIndexPage.insertKey( tmpEntry.key, 
				  ((IndexData)tmpEntry.data).getData());

	  currentIndexPage.deleteSortedRecord
	    (getIDSupplier().getID(currentIndexPage.getCurPage(), i) );
	...
	{
	  BTLeafPageI<I,T> currentLeafPage = getBTLeafPageSupplier().getBTLeafPage(page, headerPage.get_keyType() );
	  ...
	  if (currentLeafPage.available_space() >=
	      BTI.getKeyDataLength(key, NodeType.LEAF) )
	    {
	      // no split has occurred
	  ...
	  if (rightPageId.pid != INVALID_PAGE)
	    {
	      BTLeafPageI<I,T> rightPage;
	      rightPage= getBTLeafPageSupplier().getBTLeafPage(rightPageId, headerPage.get_keyType());
	  ...
	  for (tmpEntry = currentLeafPage.getFirst(firstRid);
	       ...
	       tmpEntry = currentLeafPage.getFirst(firstRid))
	    {
	      ...
	      newLeafPage.insertRecord( tmpEntry.key,
					((ILeafData<I>)(tmpEntry.data)).getData());
	  ...
	  if (BTI.keyCompare(key, undoEntry.key ) <  0) {
	    //undo the final record
	    if ( currentLeafPage.available_space() < 
		 newLeafPage.available_space()) {
	      newLeafPage.insertRecord( undoEntry.key, 
					((ILeafData<I>)undoEntry.data).getData());
	      
	      currentLeafPage.deleteSortedRecord
		(getIDSupplier().getID(currentLeafPage.getCurPage(), (int) currentLeafPage.getSlotCnt() - 1) );
	    }
	  }	  
	  ...
	  // fill upEntry
	  tmpEntry=newLeafPage.getFirst(firstRid);
	  upEntry= getKeyDataEntrySupplier().getKeyDataEntry(tmpEntry.key, newLeafPageId );
   ...
   *
   */
  public boolean Delete(KeyClass key, I rid)
  ...
  BTLeafPageI<I,T> findRunStart (KeyClass lo_key,
			   I startrid)
    throws IOException, 
	   IteratorException,  
	   KeyNotMatchException,
	   ...
	   UnpinPageException
    {
      BTLeafPageI<I,T>  pageLeaf;
      BTIndexPageI<I,T> pageIndex;
      Page page;
      BTSortedPageI<I,T>  sortPage;
      PageId pageno;
      PageId curpageno=null;                // iterator
      PageId prevpageno;
      PageId nextpageno;
      I curRid;
      IKeyDataEntry<I> curEntry;
      ...
      page= pinPage(pageno);
      sortPage=getBTSortedPageSupplier().getBTSortedPage(page, headerPage.get_keyType());
	...
	pageIndex= getBTIndexPageSupplier().getBTIndexPage(page, headerPage.get_keyType());
	prevpageno = pageIndex.getPrevPage();
	curEntry= pageIndex.getFirst(startrid);
	while ( curEntry!=null && lo_key != null 
		&& BTI.keyCompare(curEntry.key, lo_key) < 0) {
	...
	if ( trace!=null )
      ...
      pageLeaf = getBTLeafPageSupplier().getBTLeafPage(page, headerPage.get_keyType() );
	...
	pageno = nextpageno; 
	pageLeaf= getBTLeafPageSupplier().getBTLeafPage( pinPage(pageno), headerPage.get_keyType());
	curEntry=pageLeaf.getFirst(startrid);
      }
	...
	// scan will unpin it when done
      ...
      while (BTI.keyCompare(curEntry.key, lo_key) < 0) {
  ...
  private boolean NaiveDelete ( KeyClass key, I rid)
    throws LeafDeleteException,  
	   ...
	   IteratorException
    {
      BTLeafPageI<I,T> leafPage;
      I curRid=getIDSupplier().getID();  // iterator
      KeyClass curkey;
      I dummyRid;
      PageId nextpage;
      boolean deleted;
      ...
      if ( trace!=null )
	{
          trace.writeBytes("DELETE " +rid.getPageNo() +" " + rid.getSlotNo() + " "
			   + key +lineSep);
          trace.writeBytes( "DO"+lineSep);
          trace.writeBytes( "SEARCH" +lineSep);
          trace.flush();
	  ...
	  leafPage= getBTLeafPageSupplier().getBTLeafPage(pinPage(nextpage),
				  headerPage.get_keyType() );
	  entry=leafPage.getFirst(getIDSupplier().getID());
	}
	
	if ( BTI.keyCompare(key, entry.key) > 0 )
	  break;
	
	if( leafPage.delEntry(getKeyDataEntrySupplier().getKeyDataEntry(key, rid)) ==true) {
	...
	leafPage= getBTLeafPageSupplier().getBTLeafPage(pinPage(nextpage), headerPage.get_keyType());
  ...
  private boolean FullDelete (KeyClass key,  I rid)
  ...
  private KeyClass _Delete ( KeyClass key,
      ...
      if ( trace!=null )
	{
      ...
      if (sortPage.getType()==NodeType.LEAF ) {
        ...
        PageId nextpage;
        BTLeafPageI<I,T> leafPage;
        leafPage= getBTLeafPageSupplier().getBTLeafPage(page, headerPage.get_keyType());
	...
	while((tmpEntry!=null) && (BTI.keyCompare(key,tmpEntry.key)>=0)) {
          // WriteUpdateLog is done in the btleafpage level - to log the
          // deletion of the rid.
	  
	  if ( leafPage.delEntry(getKeyDataEntrySupplier().getKeyDataEntry(key, rid)) ) {
	    // successfully found <key, rid> on this page and deleted it.
		...
		return null;
	      ...
	      // get a sibling
	      BTIndexPageI<I,T>  parentPage;
	      parentPage = getBTIndexPageSupplier().getBTIndexPage(pinPage(parentPageId),
				headerPage.get_keyType());
	      
	      PageId siblingPageId=new PageId();
	      BTLeafPageI<I,T> siblingPage;
	      int direction;
		...
		return null;
              ...
              siblingPage= getBTLeafPageSupplier().getBTLeafPage(pinPage(siblingPageId),
					 headerPage.get_keyType());
		...
		BTLeafPageI leftChild, rightChild;
		if (direction==-1) {
		...
		I firstRid=getIDSupplier().getID(), insertRid;
		...
		if ( rightChild.getNextPage().pid != INVALID_PAGE) {
		  BTLeafPageI<I,T> nextLeafPage= getBTLeafPageSupplier().getBTLeafPage(
	...
	indexPage= getBTIndexPageSupplier().getBTIndexPage(pinPage(currentPageId), headerPage.get_keyType());
	...
	PageId dummyPageId; 
	KeyClass deletedKey = key;
	I curRid=indexPage.deleteKey(oldChildKey);
	
	if (indexPage.getCurPage().pid == headerPage.get_rootId().pid) {
	  // the index page is the root
	  if (indexPage.numberOfRecords() == 0) {
	    BTSortedPageI<I,T> childPage;
	    childPage= getBTIndexPageSupplier().getBTIndexPage(indexPage.getPrevPage(),
				       headerPage.get_keyType()); 
	...
	if ((4 /*sizeof slot*/ +indexPage.available_space()) <= 
	  ...
	  siblingPage= getBTIndexPageSupplier().getBTIndexPage(pinPage(siblingPageId),
				       headerPage.get_keyType());
	  
	  int pushKeySize=0;
	  if (direction==1) {
	    pushKeySize=BTI.getKeyLength
	      (parentPage.findKey(siblingPage.getFirst(getIDSupplier().getID()).key));
	  } else if (direction==-1) {
	    pushKeySize=BTI.getKeyLength
	      (parentPage.findKey(indexPage.getFirst(getIDSupplier().getID()).key));
	  }  
	  ...
	  if (siblingPage.redistribute(indexPage,parentPage, 
	    ...
	    // merge the two children
	    BTIndexPageI<I,T> leftChild, rightChild;
	    if (direction==-1) {
	      leftChild = siblingPage;
	    ...
	    // move all entries from rightChild to leftChild
	    for (IKeyDataEntry<I> tmpEntry=rightChild.getFirst(firstRid);
		 tmpEntry != null;
		 tmpEntry=rightChild.getFirst(firstRid) ) {
  ...
  public BTFileScanI<I,T,K> new_scan(KeyClass lo_key, KeyClass hi_key)
	...
	return scan;
      ...
      scan.curRid= getIDSupplier().getID();
  ...
  void trace_children(PageId id)
	...
	BTSortedPageI<I,T> sortedPage;
	I metaRid=getIDSupplier().getID();
	PageId childPageId;
	KeyClass key;
	IKeyDataEntry<I> entry;
	sortedPage= getBTSortedPageSupplier().getBTSortedPage( pinPage( id), headerPage.get_keyType());
	
	
	// Now print all the child nodes of the page.  
	if( sortedPage.getType()==NodeType.INDEX) {
	  BTIndexPageI<I,T> indexPage= getBTIndexPageSupplier().getBTIndexPage(sortedPage,headerPage.get_keyType());
	  ...
	  BTLeafPageI<I,T> leafPage= getBTLeafPageSupplier().getBTLeafPage(sortedPage,headerPage.get_keyType());
