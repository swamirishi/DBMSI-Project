...
//  }
    ...
    public static LID isLabelRecordInBtreeFound(LIDBTreeFile<Void> btreeIndexFile, String record) throws UnknownIndexTypeException, InvalidTypeException, IndexException, InvalidTupleSizeException, IOException, UnknownKeyTypeException, IteratorException, ConstructPageException, KeyNotMatchException, PinPageException, UnpinPageException, ScanIteratorException, HashEntryNotFoundException, InvalidFrameNumberException, PageUnpinnedException, ReplacerException {
        KeyClass lowKey = new StringKey(record);
        KeyClass hiKey = new StringKey(record);
        LIDBTFileScan<Void> lidbtFileScan = btreeIndexFile.new_scan(lowKey, hiKey);
        LIDKeyDataEntry lid = lidbtFileScan.get_next();
        lidbtFileScan.DestroyBTreeFileScan();
        return lid == null ? null : ((LIDLeafData) lid.data).getData();
    }
    ...
    public static <K> QID getQIDFromQIDTree(QIDBTreeFile<List<?>> qidBtreeFile, IDListKeyClassManager idListKeyClassManager, List<?> ids) throws KeyTooLongException, KeyNotMatchException, UnknownKeyTypeException, IndexException, IOException, InvalidTypeException, InvalidTupleSizeException, UnknownIndexTypeException, IteratorException, ConstructPageException, PinPageException, UnpinPageException, ScanIteratorException, HashEntryNotFoundException, InvalidFrameNumberException, PageUnpinnedException, ReplacerException {
        KeyClass lowKey = idListKeyClassManager.getKeyClass(ids);
        KeyClass hiKey = idListKeyClassManager.getKeyClass(ids);
        QIDBTFileScan<Void> qidbtFileScan = qidBtreeFile.new_scan(lowKey, hiKey);
        QIDKeyDataEntry qid = qidbtFileScan.get_next();
        qidbtFileScan.DestroyBTreeFileScan();
        return qid == null ? null : ((QIDLeafData) qid.data).getData();
    }
