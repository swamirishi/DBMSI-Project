 ...
 */
public abstract class OBufI<I extends ID, T extends Tuple> implements GlobalConst {
    /**
     * fault constructor
     ...
     */
    public void init(byte[][] bufs, int n_pages, int tSize, HFile<I, T> temp_fd, boolean buffer) {
        _bufs = bufs;
        _n_pages = n_pages;
        t_size = tSize;
        _temp_fd = temp_fd;
     ...
     */
    public T Put(T buf) throws IOException, Exception {
        ...
        try {
            ...
            System.out.println("Debug:" + curr_page + " " + t_wr_to_pg * t_size + " " + t_size + " " + copybuf.length);
            throw e;
        }
