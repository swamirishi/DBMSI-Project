 ...
 */
public abstract class SortMergeI<I extends ID, T extends Tuple> extends IteratorI<T> implements GlobalConst {
    private AttrType _in1[], _in2[];
    private int in1_len, in2_len;
    private IteratorI<T> p_i1,        // pointers to the two iterators. If the
    ...
    private IoBufI<I, T> io_buf1, io_buf2;
    private T TempTuple1, TempTuple2;
    private T tuple1, tuple2;
    private boolean done;
    private byte _bufs1[][], _bufs2[][];
    private int _n_pages;
    private HFile<I, T> temp_file_fd1, temp_file_fd2;
     ...
     */
    public SortMergeI(AttrType in1[],
                      int len_in1,
                      short s1_sizes[],
                      AttrType in2[],
                      int len_in2,
        ...
        nOutFlds = n_out_flds;
        try {
            ts_size = TupleUtils.setup_op_tuple(Jtuple,
                                                Jtypes,
                                                in1,
                                                len_in1,
                                                in2,
        ...
        if (!in1_sorted) {
            try {
                p_i1 = getSortSupplier().getSort(in1,
                                                 (short) len_in1,
                                                 s1_sizes,
                                                 am1,
                                                 join_col_in1,
        ...
        if (!in2_sorted) {
            try {
                p_i2 = getSortSupplier().getSort(in2,
                                                 (short) len_in2,
                                                 s2_sizes,
                                                 am2,
                                                 join_col_in2,
        ...
        // open io_bufs
        io_buf1 = getIoBufSupplier().getIoBuf();
        io_buf2 = getIoBufSupplier().getIoBuf();
        
        // Allocate memory for the temporary tuples
        TempTuple1 = getTupleSupplier().getTuple();
        TempTuple2 = getTupleSupplier().getTuple();
        tuple1 = getTupleSupplier().getTuple();
        tuple2 = getTupleSupplier().getTuple();
        ...
        temp_file_fd2 = null;
        try {
            temp_file_fd1 = getHFileSupplier().getHFile(null);
            temp_file_fd2 = getHFileSupplier().getHFile(null);
    ...
    public T get_next() throws IOException, JoinsException, IndexException, InvalidTupleSizeException, InvalidTypeException, PageNotReadException, TupleUtilsException, PredEvalException, SortException, LowMemException, UnknowAttrType, UnknownKeyTypeException, Exception {
        ...
        while (true) {
            if (process_next_block) {
                ...
                comp_res = compare(sortFldType, tuple1, jc_in1, tuple2, jc_in2);
                while ((comp_res < 0 && _order.tupleOrder == TupleOrder.Ascending) || (comp_res > 0 && _order.tupleOrder
                ...
                while (compare(sortFldType, tuple2, jc_in2, TempTuple2, jc_in2) == 0) {
