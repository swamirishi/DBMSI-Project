 ...
 * to clean up.
 */
public abstract class SortI<I extends ID, T extends Tuple> extends IteratorI<T> implements GlobalConst {
    private static final int ARBIT_RUNS = 10;
    private AttrType[] _in;
    private short n_cols;
    private short[] str_lens;
    private IteratorI<T> _am;
    ...
    private pnodeSplayPQI<T> Q;
    private HFile<I, T>[] temp_files;
    private int n_tempfiles;
    private T output_tuple;
    private int[] n_tuples;
    private int n_runs;
    private T op_buf;
    private OBufI<I, T> o_buf;
     ...
     */
    private void setup_for_merge(int tuple_size,
                                 int n_R_runs) throws IOException, LowMemException, SortException, Exception {
        ...
        int i;
        pnodeI<T> cur_node;  // need pq_defs.java
        
        i_buf = new SpoofIbufI[n_R_runs];   // need io_bufs.java
        for (int j = 0; j < n_R_runs; j++) i_buf[j] = getSpoofIBufSupplier().getSpoofIBuf();
...
//            i_buf[i].hf_scan.closescan();
    ...
    private int generate_runs(int max_elems,
        ...
        while ((p_elems_curr_Q + p_elems_other_Q) < max_elems) {
            ...
            cur_node = getPNodeSupplier().getPNode();
            cur_node.tuple = getTupleSupplier().getTuple(tuple); // tuple copy needed --  Bingjie 4/29/98
     ...
     * @throws SortException something went wrong in the lower layer.
     */
    private T delete_min() throws IOException, SortException, Exception {
        pnodeI<T> cur_node;                // needs pq_defs.java
        T new_tuple, old_tuple;
    ...
    */
        ...
        if (i_buf[cur_node.run_num].empty() != true) {
            // run not exhausted
            new_tuple = getTupleSupplier().getTuple(tuple_size); // need tuple.java??
	...
	*/
     ...
     */
    private void MIN_VAL(T lastElem,
                         AttrType sortFldType) throws IOException, FieldNumberOutOfBoundException, UnknowAttrType {
     ...
     * @throws UnknowAttrType attrSymbol or attrNull encountered
     */
    private void MAX_VAL(T lastElem,
                         AttrType sortFldType) throws IOException, FieldNumberOutOfBoundException, UnknowAttrType {
    ...
    public SortI(AttrType[] in,
        ...
        T t = getTupleSupplier().getTuple(); // need Tuple.java
        try {
        ...
        // of ARBIT_RUNS
        temp_files = new HFile[ARBIT_RUNS];
        n_tempfiles = ARBIT_RUNS;
        n_tuples = new int[ARBIT_RUNS];
        n_runs = ARBIT_RUNS;
        
        try {
            temp_files[0] = getHFileSupplier().getHFile(null);
        } catch (Exception e) {
            throw new SortException(e, "Sort.java: Heapfile error");
        }
        
        o_buf = getOBufSupplier().getOBuf();
     ...
     * @throws Exception       other exceptions
     */
    public T get_next() throws IOException, SortException, UnknowAttrType, LowMemException, JoinsException, Exception {
        if (first_time) {
