...
public abstract class IoBufI<I extends ID, T extends Tuple> implements GlobalConst{
  /**
   ...
   */ 
  public void init(byte bufs[][], int n_pages, int tSize, HFile<I,T> temp_fd)
    {
      _bufs    = bufs;
      _n_pages = n_pages;
      t_size   = tSize;
      ...
      flushed     = false;
      mode        = WRITE_BUFFER;
      i_buf       = getSpoofIBufSupplier().getSpoofIBuf();
      done        = false;
    }
  ...
  private  SpoofIbufI<I,T> i_buf;        // gets input from a temporary file
