 ...
 * to clean up.
 */
public class Sort extends QuadrupleIterator implements GlobalConst {
	...
	*/
    ...
    private int generate_runs(int max_elems, AttrType sortFldType, int sortFldLen)
        ...
        if (order.quadrupleOrder == TupleOrder.Ascending) {
            try {
        ...
        while (true) {
            ...
            if ((comp_res < 0 && order.quadrupleOrder == TupleOrder.Ascending) || (comp_res > 0 && order.quadrupleOrder == TupleOrder.Descending)) {
	...
	*/
     ...
     * @throws SortException something went wrong in the lower layer.
     */
    public Sort(RDFDB rdfdb, AttrType[] in,
                short len_in,
                short[] str_sizes,
        ...
        iterator.QuadrupleUtils.rdfdb = rdfdb;
